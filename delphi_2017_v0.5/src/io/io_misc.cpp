...
void CIO::setDelphiAtom(const bool& bSolvePB, const bool& bSurfCrgInSite, const string& strSizeFile, const string& strCrgFile, const string& strPdbFile, const int& iPdbFormat, const bool& bPdbUnformat, const vector<string>& strCommPDB)
{
   ...
   string strAtInf=" ",strSub0 = "",strSub1= " ";
   string strAtom = " ",strResidue="",strResidueNum="",strChain=""; 
   ...
   if (!bExistRadiiInfo) // pdb file, such as std pdb, does not have radii info
   {
      for (delphi_integer iThisAtom = 0; iThisAtom < iAtomNum; iThisAtom++)
      {
         ...
         strAtom       = strAtInf.substr(12,4); strAtom       = removeSpace(strAtom);
         strResidue    = strAtInf.substr(17,4); strResidue    = removeSpace(strResidue);
         strChain      = strAtInf.substr(21,1); strChain      = removeSpace(strChain);
         strResidueNum = strAtInf.substr(22,4); strResidueNum = removeSpace(strResidueNum);
         ...
         iFound =    (strAtom,strResidue,strResidueNum,strChain,SIZEFILE,fValue);
   ...
   for (delphi_integer iThisAtom = 0; iThisAtom < iAtomNum; iThisAtom++)
   {
      ...
      strResidue    = strAtInf.substr(17,4); 
      strResidueNum = strAtInf.substr(22,4);
