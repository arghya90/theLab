...
//ARGO - July 2017
// Added a new modification where sigma for the gaussian packing density 
// is now a funciton of the normalized atomic index of the atom.
// This is under trial right now. 
// Many more expectations rest with this modification.
...
void CDelphiSpace::setGaussian()
{
    ...
    delphi_real eps_min = 0,eps_max = 0,eps_diff = 1.0,bnd_buff = 0;
    ...
    delphi_real peak,pi = 3.14159265359,den,distance2,sigmatime = 3.0,radsq; //sigma=sigma*radius. peak=4/(3*pi**0.5*sigma**3)

    //ARGO - gaussian modification with normalized atom index 
    delphi_real fNormIndex = -1.0;
    delphi_real varSigma = sigma;
    string atom_info_string = "";
    string atom_type = "";
    ifstream avg_atomwise_rmsd(("allAtomTypes_avgRMSD.dat"));
    string at = "";
    delphi_real a_rmsd = -1.0;
    vector<string> atom_types;
    vector<delphi_real> avgRMSD;

    while (avg_atomwise_rmsd >> at >> a_rmsd ) {

                atom_types.push_back(removeSpace(at));
                avgRMSD.push_back(a_rmsd);
    }

	if (debug_space)
	{
		cout << "######## start Gaussian: #########" << endl;
		pTimer->showTime();
	}
...
//DoATOMS:
        for( iv=1; iv<=iNatom; iv++)
        {
            ...
            */
            ...
            atom_info_string = sDelPhiPDB[iv].atom_info;
            //ARGO - calculating normalized index
            //fNormIndex = ( stod(sDelPhiPDB[iv].atom_index)  minmax_AtomIndex_v[0])/(minmax_AtomIndex_v[1] - minmax_AtomIndex_v[0]);


            
            atom_type = removeSpace(atom_info_string.substr(12,4));
            atom_type = atom_type.substr(0,1);

            auto it = find(atom_types.begin(), atom_types.end(), atom_type);
            if ( it == atom_types.end()) {
                // atom type not fouund.
            } else {
                auto idx = distance(atom_types.begin(), it);
                varSigma = avgRMSD[idx] * 10;   //nm to Ang
            }
            
            cout << left << setw(20) << " Atom name " << right << setw(5) << atom_info_string.substr(12,4) << " and type " << atom_type << " is assigned a sigma of " << right << setw(15) << varSigma << endl;
...
//---------------Argo: a correction
                        //In the orioginal implementation, for gaussian density estimation, Rvdw == (Rvdw + Rprobe) was being used.
                        // I have removed that here by brining it back to Rvdw of the atoms.
                        radsq = (sDelPhiPDB[iv].radius * sDelPhiPDB[iv].radius * fScale * fScale);
                        ...
                        if (distsq<rad2aav[i123.nX].nX)
                        {
                            ...
                            den=exp(-(distance2/(varSigma*varSigma*radsq)));
                            //den=exp(-(distance2/(sigma*sigma)));
                        ...
                        if (distsq<rad2aav[i123.nZ].nZ)
                        {
                            ...
                            den=exp(-(distance2/(varSigma*varSigma*radsq))); //Gaussian: make density at center is 1.
                            //den=exp(-(distance2/(sigma*sigma)));
						...
						if (distsq < radtest)
						{
							den = exp(-(distsq / (varSigma*varSigma*radsq)));
                            //den=exp(-(distance2/(sigma*sigma)));
...
// epstemp=repsin*(1-srfcut)+repsout*srfcut
        ...
        delphi_real srfden = srfcut; //temporarily takes the value of srfcutwhich itself should be a density value
        epstemp = repsin * srfden + repsout*(1 - srfden);
		cout << left << setw(40) << " Gaussian SRFCUT at " << ":" << epstemp << endl;
            ...
            for(iy=1; iy<=iGrid; iy++)
            {
                for(iz=1; iz<=iGrid; iz++)
                {
                    if(gepsmp[ix][iy][iz].nX < srfden)
                    {
                        ...
                        gepsmp2[ix][iy][iz].nX=1 + (gepsmp2[ix][iy][iz].nX - 1)*exp((srfden - gepsmp[ix][iy][iz].nX)*(repsin - repsout));
                    }// if
                    if(gepsmp[ix][iy][iz].nY < srfden)
                    {
                        ...
                        gepsmp2[ix][iy][iz].nY=1 + (gepsmp2[ix][iy][iz].nY - 1)*exp((srfden - gepsmp[ix][iy][iz].nY)*(repsin - repsout));
                    }// if
                    if(gepsmp[ix][iy][iz].nZ < srfden)
                    {
                        //gepsmp2[ix][iy][iz].nZ=repsin;
                        //gepsmp2[ix][iy][iz].nZ=1;
                        gepsmp2[ix][iy][iz].nZ=1 + (gepsmp2[ix][iy][iz].nZ - 1)*exp((srfden - gepsmp[ix][iy][iz].nZ)*(repsin - repsout));
                    }// if
                }//do
            }//do
        }//do
...
// ifinwater=true
	...
	if (debug_space)
	{
		cout << "############ Line 754: ###########longint= " << longint << endl;
		pTimer->showTime();
	}
...
//ADDED BY ARGO
string CDelphiSpace::removeSpace(const string& strLine)
{
    string strNewLine;
