...
void CDelphiSpace::run()
{
    ...
    debug_space=true; //debug_space=true will print a lot of debug information
    const char* infoString = " Info> ";

	if (debug_space) {
		cout << "############### run in Space module.... ##################" << endl;
		pTimer->showTime();
	}
...
//    SGrid <delphi_integer> *** testEpsmap=pdc->getKey_Ptr("iepsmap",iNatom,iNatom,iNatom);
    ...
    */
    ...
    delphi_integer i,j,k,ix,iy,iz,ic,ico,natom2, iv, jv;
    delphi_real fRMid;
    ...
    //ARGO
    if ( debug_space) cout << " Creating Zeta Surface Map" << endl;
    ...
    //ARGO for distance matrix
    if ( debug_space) cout << " Creating the 2D distance2 matrix" << endl;
    get_pt2d <delphi_real> (distMatrix2, iNatom + 1, iNatom + 1);
    get_pt2d <bool> (adjMatrix, iNatom + 1, iNatom + 1);

    i = 0;
    for ( ix = 1; ix <= iNatom; ix++) {
        for ( iy = 1; iy <= iNatom; iy++) {
            distMatrix2[ix][iy] = 0.0;
            adjMatrix[ix][iy]   = false;
        }
    }
...
//######### initialize sDelPhiPDB: #########
    ...
    for (i=0; i<=iNatom-1; i++)
    {
        ...
        sDelPhiPDB[i+1].atom_info=delphipdb[i].getAtInf(); //Argo
...
//ARGO commented the ifdef in original. Place ifdef-KJI later
	...
	if (debug_space) {
		cout << "#### going out of space_run... ####" << endl;
		pTimer->showTime();
	}
